...
import {
  type KnownUser,
  recordKnownUser as _recordKnownUser,
  getKnownUser as _getKnownUser,
  listKnownUsers as _listKnownUsers,
  removeKnownUser as _removeKnownUser,
  clearKnownUsers as _clearKnownUsers,
  getKnownUsersStats as _getKnownUsersStats,
} from "./known-users.js";

// ============ 类型定义 ============

// KnownUser 类型来自 known-users.ts（单一数据源）
...
export function getKnownUser(type: string, openid: string, accountId: string): KnownUser | undefined {
  return _getKnownUser(accountId, openid, type as "c2c" | "group");
}
...
export function removeKnownUser(type: string, openid: string, accountId: string): boolean {
  return _removeKnownUser(accountId, openid, type as "c2c" | "group");
}
...
export function clearKnownUsers(accountId?: string): number {
  return _clearKnownUsers(accountId);
}
...
export function getKnownUsersStats(accountId?: string): {
  ...
  const stats = _getKnownUsersStats(accountId);
  return {
